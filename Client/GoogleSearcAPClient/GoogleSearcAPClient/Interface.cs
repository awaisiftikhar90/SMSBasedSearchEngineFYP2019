...
namespace GoogleSearcAPClient
{
    ...
    {       
        private GoogleSearchAPIProxy _googleAPIProxy;
        public Interface()
        {
            DataLayer dl = new DataLayer();
            dl.GetData();

            _googleAPIProxy = new GoogleSearchAPIProxy();
            InitializeComponent();
        }
        ...
        {           
            ...
            var response = _googleAPIProxy.GetGoogleSearchData(keyword);

            var result = response.FirstOrDefault();

            string firstResult = result.Text + result.Link;
            sendMessage(firstResult, number);
            return response;
        }
        ...
        private void Message_Received(object sender, MessageReceivedEventArgs e)
        {
            try
            {
                ...
                if (obj is MemoryLocation)
                {
                    ...
                    string num = msg.OriginatingAddress;
                    GetGoogleSearchData(ss, num);
